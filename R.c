...
void game_menu(Map *map, Player *player) {
    initscr();
    Mix_Music *bgMusic = Mix_LoadMUS("C:\\Users\\amir8\\Downloads\\interstellar.Day-one(320).mp3");
    if (bgMusic == NULL) {
    ...
    while ((ch = getch()) != 'q') {
        ...
        switch (ch) {
            ...
            case 's': map->show_full_map = !map->show_full_map; break; // تغییر عملکرد کلید s
